...
```{r}
g1<-data %>%
  filter(data$BedroomAbvGr=='0') %>%

g2<-data %>%
  filter(data$BedroomAbvGr=='1') %>%

g3<-data %>%
  filter(data$BedroomAbvGr=='2') %>%

g4<-data %>%
  filter(data$BedroomAbvGr=='3') %>%

g5<-data %>%
  filter(data$BedroomAbvGr=='4') %>%
g6<-data %>%
  filter(data$BedroomAbvGr=='5') %>%

g7<-data %>%
  filter(data$BedroomAbvGr=='6') %>%
g8<-data %>%
  filter(data$BedroomAbvGr=='7') %>%
g9<-data %>%
  filter(data$BedroomAbvGr=='8') %>%



t.test(g0$BedroomAbvGr, g1$BedroomAbvGr, alt = "greater", var.equal = TRUE)
t.test(g0$BedroomAbvGr, g2$BedroomAbvGr, alt = "greater", var.equal = TRUE)
t.test(g0$BedroomAbvGr, g3$BedroomAbvGr, alt = "greater", var.equal = TRUE)
t.test(g0$BedroomAbvGr, g4$BedroomAbvGr, alt = "greater", var.equal = TRUE)
t.test(g0$BedroomAbvGr, g5$BedroomAbvGr, alt = "greater", var.equal = TRUE)
t.test(g0$BedroomAbvGr, g6$BedroomAbvGr, alt = "greater", var.equal = TRUE)
t.test(g0$BedroomAbvGr, g7$BedroomAbvGr, alt = "greater", var.equal = TRUE)
t.test(g0$BedroomAbvGr, g8$BedroomAbvGr, alt = "greater", var.equal = TRUE)
t.test(g0$BedroomAbvGr, g9$BedroomAbvGr, alt = "greater", var.equal = TRUE)
...
Svaka prodana nekretnina nalazi se u određenom naselju i ima određenu veličinu podruma. Zanima nas razlikuju li se uspješnosti prodaje nekretnina u određenom naselju s obzirom na veličinu podruma. Za početak želimo vidjeti ravnaju li se veličine podruma svake prodane nekretnine u određenom naselju po normalnoj razdiobi kako bi mogli primijeniti analizu varijance.
```{r out.width=c('50%', '50%'), fig.show="hold"}
...
```
8>
Iz rezultata ANOVA testa definitivno možemo zaključiti da sredine tih uzoraka nisu jednake te odbaciti nultu hipotezu u korist tvrdnje da su sredine različite.
ANOVA nam samo govori da su sredine tih kategorija međusobno različite, 

## Utjecaj godine prodaje na cijenu nekretnine:

```{r echo=FALSE}
boxplot(data$SalePrice[data$YrSold != "<undefined>"] 
        ~ data$YrSold[data$YrSold != "<undefined>"],
        ylab= "velicina podruma",
        xlab= "godina prodaje", col=rainbow(3))
...
```{r}
result = cor(data$SalePrice, data$YrSold, method = "pearson")
 
# Print the result
cat("Pearson correlation coefficient is:", result)
```

Pearsonov koeficijent pokazuje da su varijable nekolinerne odnosno
nezavisne.
